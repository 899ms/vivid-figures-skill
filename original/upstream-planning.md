...
新整套先运行 `node --experimental-strip-types "<SKILL>/scripts/figure-plan-cli.mjs" begin --workspace "<WORKSPACE>"`。读取题目、逐问建模、机器结果、验证和证据，先规划后绘制。读 resources/assets/shared-scripts/figure_exemplars.md 的方法套餐及 resources/assets/shared-scripts/figure_style_guide.md 的数据形态决策表。以真实分析步骤推导图集，不从旧图片倒推，也不复用固定六图清单。
