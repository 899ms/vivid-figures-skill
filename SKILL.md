...
4. 先读取 [模式与配色选择](color-selection.md)，在鲜艳版、稳重版两个入口下均提供配色选项；尚未选择时按该说明询问用户。用户选择“鲜艳舒适型/expressive”时完整读 [expressive](original/profiles/expressive.md)；选择“稳重科研型/restrained”时完整读 [restrained](original/profiles/restrained.md)，按配色选择及宿主适配设置原有项目标记。仅覆盖默认颜色搭配，其余模式指导保持不变。
